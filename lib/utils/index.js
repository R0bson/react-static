...
function glob(path) {
  ...
  return new Promise(function (resolve, reject) {
    return (0, _glob2.default)(path, options, function (err, files) {
      if (err) {
        return reject(err);
      }
      resolve(files);
...
function time(message) {
  times[message] = _perf_hooks.performance.now();
}
function timeEnd(message) {
  if (times[message]) {
    console.log(message + ' (' + Math.round((_perf_hooks.performance.now() - times[message]) * 10) / 10 + 'ms)');
    times[message] = null;
  }
}
